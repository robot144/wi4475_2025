...
# ╔═╡ 902c8f7b-1e03-49aa-b599-6cfc53ca0bb2
md"""
...
![image of a spring-mass system from wikipredia](https://upload.wikimedia.org/wikipedia/commons/2/25/Animated-mass-spring.gif)
"""

# ╔═╡ 7ba19d32-f423-421e-87ce-d149de90525a
md"""
### 1a: Equilibrium

Show that for $r>=0$ and $F>0$ there is one equilibrium. Note that the question is not to try to solve the equation analytically with the Cardano formula.
Also show that for $r<0$ and $0<F<F_{critical}$ the equation has three real roots.
And compute $F_{cricital}$.
"""
...
# ╔═╡ 86679cb4-42e4-4b11-9208-77361d3cd05e
md"""
### 1b: Find root numerically

Let's try to find a good approximation of the equilibrium for $x$ numerically. For this we use newton iterations, but in a slightly different form:
- substitute $x_i+\Delta x_i$ for $x$ in the equations for the equilibrium and solve for $\Delta x_i$ while ignoring terms in $\Delta x_i$ higher than $O(\Delta x_i)$.
- solve for $\Delta x_i$
- implement 
- apply for $F=0.2$, $r=-0.1$
"""

# ╔═╡ 0385b1dd-c975-4738-b239-31186464a0a9
md"""
__Answer__
...
# ╔═╡ 421a1b91-a75d-4aa6-9e32-23fa79d7e82c
begin
	#settings
	F=0.2
	r=-0.1
...
# ╔═╡ a15d3e05-00c6-468b-b8a1-b9b7d4c1db16
begin
...
# ╔═╡ 61bd16f8-9f0d-41ff-9524-7c9064fc1b9d
md"""
### 1c 
...
where $\mathbf{x}(t)$ is the state vector. Here we use $x$ and $x'$ as elements of the state vector. Write in this form and make an analysis (analytical) of the stability. How would you characterize the behaviour of the system?
"""

# ╔═╡ 99df5781-94a6-4834-a14d-18b632a8adf5
md"""
__Answer__
...
# ╔═╡ e2e4dca7-c483-4697-a741-76145e5d85a8
md"""
### 1d
...
# ╔═╡ 07160a4c-c9d2-4443-8c50-340e85d06714
...
# ╔═╡ 5eeb7e6b-fe40-4c7a-8ec1-a18181feb056
md"""
__Answer__
...
# ╔═╡ eb336c8c-a1f0-49d8-b3c3-c8108c20726d
md"""
### 1e 

An improved second-order method is known as Heun. Make an analysis of the stability of the resulting discrete-time system. In what ways is this method better than Euler? What is the difficulty, so what must be going on in the background of the Heun time-integration in teh example below?
"""

# ╔═╡ 0ed2a6da-f85c-4ac5-8dbd-ad2df1085d86
# solver using Heun
begin
	prob_heun = SecondOrderODEProblem(nonlinearoscillator, dx₀, x₀, tspan, p)
...
# ╔═╡ 2d7f4276-b832-4042-afe9-547412bc4f3a
md"""
__Answer__
...
# ╔═╡ c1d78024-a0f3-4ca9-9cd9-7bda351c9b27
md"""
## Question 2: Linear regression / least-squares

In this question we'll study sealevel rise at the location Vlissingen in the south-western part of the Netherlands. Vlissingen has one of the longest tide gauge records in the country. A common way to study sealevel rise considers the annual averages of the measured data. In practice also corrections are needed e.g. to compensate for subsidence of the tide gauge. The [PSMSL](https://psmsl.org/) aims to maintain a worldwide collection of tide guage measurements exactly for this purpose. Let's download and plot the data first.
"""

# ╔═╡ a2c02d1e-5149-4feb-bff2-0e9bc58086ef
...
begin
	...
	plot(series.Year,series.SeaLevel,xlabel="year",ylabel="sealevel [mm]", title="Annual mean sealevel at $(station_label)",label=false)
end
...
Let's consider the followind linear model with noise: 

$z(k) = z_{2000} + \alpha (t(k)-2000) + \epsilon(k)$

Where $z(k)$ is the measured sealevel, $t(k)$ is the year, and $\epsilon(k)$ is assumed to be white Gaussian noise with mean zeros and standard-deviation $\sigma$.

Rewrite this problem, such that we get the cost-function:

$J(\mathbf{x}) = \frac{1}{2} (\mathbf{z}-\mathbf{H}\mathbf{x})'\mathbf{R}^{-1}(\mathbf{z}-\mathbf{H}\mathbf{x})$

where $\mathbf{x}=[z_{2000},\alpha]'$ is a vector with the parameters of the model and $\mathbf{z}$ is a vector that contains all the measurements. Finally, find the optimal $\mathbf{x}$. Assume $\sigma=1.0$ to get started.
"""

# ╔═╡ 6431f059-a21f-4a51-8804-f02f447efd35
md"""
__Answer__
"""

# ╔═╡ 8d1710b8-3478-4f56-9689-99a8409906b9
#code
begin
	z=series.SeaLevel
	t=series.Year
...
Next estimate $\sigma$ using the residuals and use the Hessian of the cost-function to estimate the error covariance of the estimated parameters.
"""

# ╔═╡ 7757f681-0d82-45a2-b6f7-4db700b4a6b8
md"""
__Answer__
...
# ╔═╡ 2195149d-5947-4e83-8233-7aaa8274a996
#code
begin
	#TODO
end
...
# ╔═╡ 1d282971-2882-46b4-89fc-17b4ea32df28
md"""
__Answer__
...
# ╔═╡ b9ae0552-f0fb-4710-bd74-242f94e33c6c
md"""
### 2d: Correction for effect of local wind and pressure

The sealevel at a coastal location also varies with the wind and air-pressure. It is possible to use a model (numerical or statistical) to compensate for these variations. Here we use data from the [Global Tide Surge Model](https://www.deltares.nl/en/expertise/projects/global-modelling-of-tides-and-storm-surges) as also used by the "sealevelmonitor"

Fit the model to the corrected series. Estimate the resulting $\sigma$. What is a benefit of this approach?
"""

# ╔═╡ 3f841bf3-f2cd-4a57-911d-4f2b50948d57
#code
begin
	function get_gtsm_data(station_name)
		url = "https://raw.githubusercontent.com/Deltares-research/sealevelmonitor/refs/heads/main/data/deltares/gtsm/gtsm_surge_annual_mean_main_stations.csv"
		println(url)
		surge_series = CSV.File(HTTP.get(url).body, header=3,dateformat="yyyy-mm-dd") |> DataFrame # download data and parse
		surge_series.Year = Dates.year.(surge_series.t) # get year from date 
		surge_series=filter(row -> row.name==station_name, surge_series) # select station
	...
	# plot 
	p1=plot(year_surge,surge,xlabel="year",ylabel="surge [mm]",label="correction", title="Surge correction at $(station_label)")
	p2=plot(series.Year,series.SeaLevel,xlabel="year",ylabel="sealevel [mm]", title="Annual mean sealevel at $(station_label)",label="measured")
	plot!(p2,series.Year,z_corrected,label="corrected")
	plot(p1,p2,layout=(2,1))
end

# ╔═╡ d3d1f7d8-27b3-4b0b-a6d6-2e7e0ffb1d0c
md"""
__Answer__
"""

# ╔═╡ 2907b338-af29-4c68-97dc-bd01592d56c4
begin
	#TODO
	@show year_surge, surge, z_corrected
end

# ╔═╡ 8ad0661f-4b40-4844-a3ff-bac59a276452
md"""
...
Use your estimates of $\sigma$, $z_{2000}$ and $\alpha$ to generate a synthetic dataset with the model. Then estimate the parameters again and repeat this many (e.g. 100) times. The covariance of the parameters can be computed from these samples.
"""

# ╔═╡ 7f41de4d-83e3-4e59-aa2b-c2aed45b6038
md"""
__Answer__

"""

# ╔═╡ e97cf559-df9b-4f8f-8faa-fb5787582eba
begin
	#TODO
end
...
manifest_format = "2.0"
project_hash = "f57924871db01c9dd7a9dcef8bd4b68787a718cb"
