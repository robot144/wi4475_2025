...
using Plots, ForwardDiff, DifferentialEquations, ReverseDiff, Interpolations, Statistics

# ╔═╡ 188caa27-ae42-40c3-a971-6e4ccb80bd3d
md"""
![TU Delft logo](https://raw.githubusercontent.com/robot144/wi4475_2024/main/images/tu_logo.png)
# Homework series 2 for Data Assimilation course (WI4475 2025)

This is the second homework series for the data assimilation course (WI4475 2025). There are 2 questions with 5 subquestions each. You can earn one point for each of these 10 subquestions, which directly sums up the grade for this homework series. You can select your programming language and editor of choice, but we can only process PDF files for grading (unfortunately). However, the interactive elements in this document will only work with Pluto.jl and it's probably easiest to add the answers after each question and generate a pdf from there using the ![export button](https://raw.githubusercontent.com/robot144/wi4475_2025/main/images/export_small.png) on the top right of the screen. 

The files for this homework series are all available [here](https://github.com/robot144/wi4475_2025).
"""
...
With 'ctrl-m' you turn a cell into a text cell. The text is written in [markdown format](https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax) 	

In the Pluto notebook, the code can be hidden and shown again by clicking on the eye symbol:
![eye symbol](https://raw.githubusercontent.com/robot144/wi4475_2024/main/images/eye_small.png)

Just try to show the markdown code for this block of text now.
"""

# ╔═╡ c073733c-929a-4aad-8a05-82936ec27e53
md"""
...
The [Lorenz equation](file:///home/verlaan/Downloads/1_2_intro_l95.pdf) is an Ordinary Differential Equation (ODE) that describes a weather variable around our planet at a fixed latitude. It is a very simplified model, yet it shows some interesting chaotic behavior. The Lorenz 95 system is given by:

$\frac{d u_m}{d t}=-u_{m-2}u_{m-1}+u_{m-1}u_{m+1}-u_m+F$
...
# ╔═╡ 6dfb5c82-0e17-40bd-b353-f03ada9a03e9
md"""
Answer ...
"""

# ╔═╡ e8442a00-b467-498b-a3a4-ae8cf28a5570
md"""
### 1b: Lagrange multipliers 

Assume that we observe all state variables at time $T$ with some noise, i.e. 
$Z_m(T)=u_m(T)+v_m$, where $v_m$ are independent with variance $\sigma^2$.
...
# ╔═╡ 8a02bfb1-9959-4b5c-868f-c33f79a2ee46
md"""
Answer ...
"""

# ╔═╡ 662bf348-e170-4851-bee7-8fffa73910f6
md"""
### 1c: Adjoint solution 

Continuing on the previous result, find an expression for the adjoint-model and the gradient of $J$ with respect to the initial condition $c_m ~m \in 1,\ldots,M$.
First, develop an expression for $\delta J$ and apply integration by parts. 
"""

# ╔═╡ eb35fe1a-1feb-4f6b-bd78-4d5c4a62c926
md"""
Answer ...
"""
...
# ╔═╡ 043960b1-d140-4cc3-bd2e-6a091e14431b
md"""
Answer ...
"""

# ╔═╡ 8213cf25-4630-4f44-ac31-31e2b33192de
md"""
...
On way of solving this, is by extending the state-vector $\mathbf{x}_{k}$ with the additional variable $F$. Show how this can be used to reshape the problem into a form where we estimate again the initial condition, albeit of an augmented system.
"""

# ╔═╡ 4c927a2d-51eb-44ad-9382-188c26f77bd8
md"""
Answer ...
"""

# ╔═╡ d1631034-a37d-4104-91a5-5200d1f1d827
md"""
...
# ╔═╡ 66ee4af6-6cab-4408-a0a3-7ab8cad4994d
md"""
...
# ╔═╡ 32265c30-36fc-4615-9aef-b78232e69387
md"""
...
$\frac{\partial u_k}{\partial t} = \frac{\tau_{k+1}-\tau_{k}}{\Delta z}$

with $\tau_k = \nu \frac{u_{k}-u_{k-1}}{\Delta z}$

Next we collect all $u_k$ into a vector $\mathbf{u}$, so we can use an ODE-solver. The example below gives all the details.
"""

# ╔═╡ bcc73656-0a50-44b4-a37c-4f66931e28fa
md"""
...
# ╔═╡ a285fbd3-896a-4cab-9a4c-a4b0fa9d89f6
md"""
...
# ╔═╡ 18ec91f6-33fe-49bf-8885-1d3b4cd0ad36
md"""
Answer ...
"""

# ╔═╡ b2ee7464-641f-4ae4-a78c-cb94da649c49
begin
	# your code
end

# ╔═╡ 509df81c-4805-4177-9e9d-9ab6bab2fa1e
md"""
...
Adapt the example below, so it computes the derivative $\partial \mathbf{u}/\partial \tau_s$ for $\tau_s=1.0 ~ N/m^2$. Also compare the result to your answer in 2a.
"""

# ╔═╡ 3c1990b2-bde8-465c-91f6-3cb2cfd9d6ce
md"""
Answer ...
"""

# ╔═╡ 1ae4bdac-ef5e-4f17-bcad-173e00cfd191
md"""
...
$h(x+\epsilon)=x^2+2 x \epsilon + \epsilon^2$
...
# ╔═╡ 6100ebb9-dcde-439d-9591-0ce3bf12224c
begin
	# This code was based on: https://book.sciml.ai/notes/08-Forward-Mode_Automatic_Differentiation_(AD)_via_High_Dimensional_Algebras/
	...
	Base.:+(f::Dual, α::Number) = Dual(f.val + α, f.der) # for expressions Dual(a,b)+2.0
	Base.:+(α::Number, f::Dual) = f + α 

	Base.:sin(f::Dual) = Dual(sin(f.val), cos(f.val) * f.der)

	# Compute 
	function df_dx_dual(f::Function, x::Number)
    	y = f(Dual(x, 1.0))
    	return y.der
	end
	...
	@show df_dx_dual(f_1, π/3)
end

# ╔═╡ 82ced850-c715-4c5f-9f5c-08863181054e
begin
	# Complete where you see TODO 
	const TODO=0.0 # just a trick to keep incomplete code from crashing

	# First terms of Taylor series for sin(x)
	f_2(x)=x - x^3/factorial(3) + x^5/factorial(5) - x^7/factorial(7) + x^9/factorial(9)

	# to power integer, like x^3
	Base.:^(f::Dual, n::Integer) = Dual(f.val^n, TODO)

	# division of two Dual numbers
	Base.:/(f::Dual, g::Dual) = Dual(f.val / g.val, TODO)

	# division of Dual number by a normal number
	Base.:/(f::Dual, α::Number) = Dual(f.val / α, f.der / α)

	# subtraction f - g
	Base.:-(f::Dual, g::Dual) = Dual(f.val - g.val, TODO)

	@show df_dx_dual(f_2, π/3)
end

# ╔═╡ fc669c2c-6ac1-451b-a876-852e4c97bbde
begin
    #plotting and animation for Lorenz equation
    function plot_lorenz(u, t)
        n=length(u)
        x=collect(1:n)
        plot(x, u, title="t=$(t)",label=false, ylims=(-10,15))
        xlabel!("x")
        ylabel!("u")
    end
    
    function animate_lorenz(sol, timestep=0.05, fps=15)
        # create an animation
        times = sol.t[1]:timestep:sol.t[end] # times at which to plot the solution
        nt = length(times)
        anim = @animate for i in 1:nt
              u=sol(times[i])
              plot_lorenz(u,times[i])
        end
    ...
    function lorenz(du_dt, u, p, t)
        F=p[1]
        M=length(u)
        for m in 1:M
            m_minus_1 = wrap(m-1,M)
            m_plus_1 = wrap(m+1,M)
            m_minus_2 = wrap(m-2,M)
            du_dt[m]=-u[m_minus_2]*u[m_minus_1]+u[m_plus_1]*u[m_minus_1]-u[m]+F
        end
    end
...
# ╔═╡ 5cda31b8-bebf-49b8-824f-e02e03ca9a55
begin
    # compute z grid at u cell centers
    function z_u(H,n)
        Δz=H/n
        z=collect(((1:n).-0.5)*Δz) # z values for u (midpoints of the cells)
        return z
    end

    #plotting and animation for velocity profile equation
    function plot_profile(u, t, p)
		H=20.0
        n=length(u)
        z=z_u(H,n)
        plot(u, z, title="t=$(round(t/3600,digits=2)) hours",label=false, ylims=(0,H),xlims=(0.0,1.0))
        xlabel!("u")
        ylabel!("z")
    end
    
    function animate_profile(sol, p, timestep=200.0, fps=15)
        # create an animation
        times = sol.t[1]:timestep:sol.t[end] # times at which to plot the solution
        nt = length(times)
        anim = @animate for i in 1:nt
...
function profile(du_dt, u, p, t)
    κ=0.4       # von Karman number
    H=20.0      # depth of the fluid
    c_b=0.01    # bottom friction coefficient
    ρ=1000.0    # density of the fluid
    ν_max=0.2   #maximum eddy viscosity
    τ_s_values=p #wind stress is a formal parameter of the model
    tspan=(0.0,5*3600.0) #time span of wind stress function
    n=length(u)
    Δz=H/n
    τ_b=c_b*u[1]^2 #bottom stress from velocity near the bottom
	# make function for interpolation of wind stress
    nt=length(τ_s_values)
    Δt=(tspan[2]-tspan[1])/(nt-1)
    t_values=collect(0.0:Δt:(5*3600.0))
    τ_s_function=linear_interpolation(t_values, τ_s_values, extrapolation_bc=Line())
	
    ν=similar(u,n) #eddy viscosity (array with elements of same type as u)
    for k in 1:n
        z=(k-1)*Δz
        ν[k]=ν_max*4.0*κ*z/H*(1-z/H)
    end
    τ=similar(u,n+1) #stress profile
    τ[1]=τ_b #bottom stress
    τ[n+1]=τ_s_function(t)/ρ #surface stress - ! time dependent
    for k in 2:n
        τ[k]=ν[k]*(u[k]-u[k-1])/Δz #stress from velocity gradient
    end
    for k in 1:n
        du_dt[k]=(τ[k+1]-τ[k])/Δz # momentum balance
    end
end

    
    τ_s=1.0     # surface stress
    p_profile=[τ_s, τ_s] # parameter vector has value at beginning and end
    n_profile=40 # number of grid cells in profile
    u₀_profile = zeros(n_profile) # initial condition
    tspan_profile=(0.0,5*3600.0)

    prob_profile=ODEProblem(profile,u₀_profile,tspan_profile,p_profile)
    sol_profile=solve(prob_profile)
    animate_profile(sol_profile,p_profile)
end

# ╔═╡ a29ffb72-699e-4630-a76b-0cd6fb581069
begin
# Wrap model in a function mapping stress to velocity at time T=5 hours
function stress_to_u5(τ_s)
    p=[τ_s, τ_s] #parameters, here wind-stress at begin and end of tspan
    n=40 # number of grid cells
    u₀ = zeros(n) # initial velocities
    tspan=(0.0,5*3600.0)

    prob=ODEProblem(profile,u₀,tspan,p)
    sol=solve(prob)
    return sol(5*3600.0)
end

@show u5=stress_to_u5(1.0) # evaluate at tau=1.0 N/m^2

@show mean(u5)
	
end

# ╔═╡ d4397d69-d351-48d2-854e-a7b48f4dd8df
begin
	f_test(x)=[x,sin(x),cos(x)]
	df_dx=ForwardDiff.derivative(f_test,π/6) # second argument is point for derivative
end

# ╔═╡ a5030603-18e9-4da3-be52-eb1965dbfa33
md"""
...
# ╔═╡ eb4d1f52-627d-423d-8b12-739e6a1cbffd
md"""
Answer ...
"""

# ╔═╡ 4bec4c24-e57f-4424-a86e-43cbe1936aeb
md"""
### 2e: Reverse mode
...
As an example consider $τ_s(t)$ as a function of time and map to the depth-averaged flow at $t=5 ~ hours$, i.e. $\bar{u}(T)$. For the representation of $τ_s(t)$ we use a vector $[τ_s(t_1), \ldots, τ_s(t_m)]$ and linear interpolation. 

The code below shows a simple example of how to compute the gradient of a function with a reverse mode method. 
"""

# ╔═╡ 8fa1aacd-f464-48a6-9270-44742a9813a6
begin
	# example function
	g(x)=x[1]+x[2]^2+x[3]^3

	x_ref=[1.0,1.0,1.0]
	gradient_g = ReverseDiff.gradient(g,x_ref) #args: function, input to func

	@show gradient_g
end

# ╔═╡ 015d351a-331f-4429-9842-c8bee3e04ba4
md"""
...
# ╔═╡ 08c0278a-2606-4681-ad6b-b1ba2e6d5066
begin
function stress_series_to_umean(τ_s_values)
    p=τ_s_values #parameters
    n=40
    u₀ = zeros(n)
    tspan=(0.0,5*3600.0)

    prob=ODEProblem(profile,convert.(eltype(τ_s_values),u₀),tspan,p)
    sol=solve(prob,Tsit5(),sensealg=SensitivityADPassThrough())
    umean=mean(sol(tspan[2]))
    return umean
end

t_values=collect(3600.0.*(0:(1/6):5))
τ_s_values=1.0.*ones(length(t_values)) # reference is 1.0 Nm^-2 for all times
τ_s_function=linear_interpolation(t_values, τ_s_values, extrapolation_bc=Line())
	
umean=stress_series_to_umean(τ_s_values) # evaluate at tau=1.0
@show umean

gradient_umean = ReverseDiff.gradient(stress_series_to_umean,τ_s_values)
@show gradient_umean

end

# ╔═╡ 99d0f993-08f5-4bb0-a4fd-0fc221c65fd9
begin
	plot(t_values/3600.0,gradient_umean,xlabel="time [hours]",ylabel="gradient du/dτ_s(t_i)",label=false)
end

# ╔═╡ a8fd6f89-4d55-42cc-ae1f-97a44444ed9b
md"""
If we make a small perturbation $\Delta \mathbf{\tau}_s=[\Delta \tau_s(t_1),\ldots, \Delta \tau_s(t_N)]$ to the stress time-series $\mathbf{\tau}_s=[\tau_s(t_1), \ldots, \tau_s(t_1)]$, then the impact can be estimated using the gradient. Create a vector $\Delta \mathbf{\tau}_s$ based on a Gaussian bump and compare the estimate to the actual change in the mean velocity at final time.
"""

# ╔═╡ 8b2fb706-73c1-4bb0-b490-b30892fc7a72
md"""
Answer ...
"""
...
manifest_format = "2.0"
project_hash = "22b6ce275068a8dca5e7a4f297ddcf31d1c60ed2"
...
git-tree-sha1 = "087720e4391faac4bc5b5f4315794b3309264c6f"
...
git-tree-sha1 = "7e5d8ce9fa2ef8f414bb5fd8aafd1c384785cafc"
...
git-tree-sha1 = "267516d68159ba75f6430fcda1dd6d771cfba74c"
...
deps = ["DiffEqBase", "FastBroadcast", "MuladdMacro", "OrdinaryDiffEqCore", "Polyester", "RecursiveArrayTools", "Reexport"]
git-tree-sha1 = "d62c8e92b70fb9273444577bb4d45e4b60a94a66"
